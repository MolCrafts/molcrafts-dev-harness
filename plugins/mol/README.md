...
|---|---|---|---|
| `/mol:bootstrap` | Initialize or maintain the agent harness. Writes `.claude/notes/law.md` — the **project constitution** (not a style guide): 11 laws in Principle / Intent / Never / Derived guidance, ordered shape → cuts → surface → truth → evolution → verification. Exceptions recorded; YAGNI/SOLID/DRY are derived heuristics, not laws. Plus the project's own invariants. No second overridable tier — a carve-out exists only where `law.md` names the exempt subsystem. Update path **appends** a missing default law id, never rewords an existing one. CLAUDE.md indexes it one line per law and stays ≤ ~100 lines. Three paths: create / audit+repair / no-op. Never writes project source; never rewords a law. | First time in a project; after upgrading mol; when harness has drifted. | `/mol:bootstrap` |

### 1 — Plan & specify

| Skill | What | When | Example |
|---|---|---|---|
| `/mol:discuss <topic>` | Design discussion + Convergence pulse. **Tier A**. Loads `law.md`; a law-violating alternative is not an Open option. Converge → auto `/mol:grill` (plan); discard → no artifacts. 8-turn cap. Never auto `/mol:spec` (tier C: 落盘 / 写 spec). | Requirement not clear enough for `/mol:spec`. | `/mol:discuss should /mol:perf own remote bench runs?` |
| `/mol:grill [mode] <plan\|slug>` | One-question interview (**tier A** when plan exists). Loads `law.md`; does not offer a waiver. **plan** → sharpened plan + tier C handoff; **spec-audit** → `clean \| supersede_needed` (law holes in the written Design are `supersede_needed`). Auto from discuss/spec; free-form 盘问/grill. | Stress-test a plan or written spec. | `/mol:grill mode:spec-audit morse-bond` |
| `/mol:spec` | Requirement → spec + acceptance. Persist gated on `architect` `mode: design` vs `law.md` (🚨/🔴 citing a law id never land). Then auto grill (spec-audit), clean → auto `impl-all`. **Tier C** — 落盘 / 写 spec; never silent from discuss/grill. Law is a constraint every coding verb inherits, not a sibling skill. | Binding artifact for non-trivial work. | `/mol:spec add Morse bond potential to molpy` |
...
| Skill | What | When | Example |
|---|---|---|---|
| `/mol:review` | The unified multi-axis static reviewer. Fans out to one single-axis agent per axis, hands findings to the `reviewer` agent for the table + verdict. Use `--axis=<name>` to scope to one dimension: `arch`, `perf`, `docs`, `ux`, `api`, `science`, `numerics`, `visual`, `security`, `ffi`, `hygiene`. On production or test source, `architect` is **non-optional** even if `--axis` omits `arch` — `law.md` cannot be opted out. Docs-only scope follows `--axis`. Surfaces runtime evaluator handoffs (`/mol:perf`, etc.) when an `acceptance.md` is in scope. | Before commit / push / PR; or when you want one specific axis checked. | `/mol:review` &nbsp;·&nbsp; `/mol:review --axis=security` &nbsp;·&nbsp; `/mol:review morse-bond` |

...
| `/mol:compact [<scope>]` | **Harness memory compaction.** Sweeps every claim in `CLAUDE.md` + `.claude/notes/**` + `.claude/specs/INDEX.md` and drops what the repo no longer supports: dead paths/symbols/scripts, claims the config contradicts, rules stated in two places, superseded pairs. Staleness is decided by **evidence, never by age** — a claim it cannot check is kept and listed, never guessed away. Writes only after approval. Free-form: 压缩记忆 / harness 臃肿了 / 清理过时的. | When the harness has drifted from the code — after a refactor that deleted modules, or when notes start describing things that are gone. | `/mol:compact` &nbsp;·&nbsp; `/mol:compact .claude/notes/release.md` |

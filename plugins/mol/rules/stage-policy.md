...
- `/mol:impl` defaults to **additive** changes. A public-signature
  change needs an explicit major bump, or a deprecation shim in the
  same diff. Impl does not call `/mol:simplify` and does not pause
  for approval.

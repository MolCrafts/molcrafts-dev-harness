...
libraries and the `molcrafts-dev-harness` marketplace alike; it adapts per repo
...
Commit gate: `pre-commit run` once, inside `/mol:commit`. It does not call `/mol:ship`.
Push gate: `pre-commit run --all-files` once, then `/mol:ship push` (`$META.build.test` once).

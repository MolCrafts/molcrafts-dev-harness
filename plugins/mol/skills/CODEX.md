...
3. Use the host subagent. Do not pass a model name.
4. Read-only agents do not write. `tester` writes MEDIUM/LARGE tests. `implementer` writes production and, in `mode: small` or `mode: fix`, the one new test. The skill owns gates and ledgers.

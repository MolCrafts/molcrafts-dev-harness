...
`molcrafts-dev-harness` declares `bump_skill: release-bump` + `gate_skill: check`
...
**3c. Harness** — if sibling `molcrafts-dev-harness` exists: dirty or untagged commits that this package's harness relies on → BLOCK until the harness is released (run `/mol:release` in the `molcrafts-dev-harness` checkout). Missing checkout → 🟡 skip.
